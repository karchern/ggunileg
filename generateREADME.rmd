...
Note how the `shape_label` does not get used here, instead the initial `color_label` variable is mapped to both color and shape aesthetics here. 
...
The number of distinct shape levels can be controlled using the `num_shape_levels` parameter which expects an intenger. In this case, shapes default to (in my opinion) nice-looking shapes.
...
Alternatively you can control the shapes itself with the `shape_values` which gets repeated for each . 
